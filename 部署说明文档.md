...
如果使用自定义路径（在 .env 中配置），请确保目录存在且有写入权限：
```bash
...
# 数据存储路径（可选，默认为项目目录下的 data 文件夹）
# DATA_DIR=/var/lib/briefy/data

# 临时文件路径（可选，默认为项目目录下的 temp 文件夹）
...
# 摘要新闻来源（逗号分隔，默认：华尔街见闻,财联社,金十数据）
# SUMMARY_SOURCES=华尔街见闻,财联社,金十数据

# Reader API 配置（用于网页内容提取）
...
**注意**：
- 如果不使用 AI 摘要功能，保持 `ENABLE_SUMMARY=0`
- 如果不使用语音播报功能，保持 `ENABLE_TTS=0`
- 如果修改了 `DATA_DIR` 或 `TEMP_DIR`，确保对应目录存在且有写入权限
...
server {
    ...
    # SSL 证书（使用 Let's Encrypt 时会自动配置）
...
访问 `https://your-domain.com` 查看服务状态。
